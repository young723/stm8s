...
void qmp6988_set_power_mode(unsigned char power_mode)
{
...
#if defined(USE_SPI)
	qst_spi_read(QMP6988_CTRLMEAS_REG, &reg_data, 1);
#else
	...
	reg_data |= power_mode;
#if defined(USE_SPI)
	qst_spi_write(QMP6988_CTRLMEAS_REG, reg_data);
#else
...
{
#if defined(USE_SPI)
	qst_spi_write(QMP6988_IO_SETUP_REG, standby<<5);
#else
...
#if defined(USE_SPI)
		qst_spi_write(QMP6988_CONFIG_REG, reg_data);
#else
...
#if 1
#if defined(USE_SPI)
	qst_spi_read(QMP6988_CALIBRATION_DATA_START,a_data_u8r,QMP6988_CALIBRATION_DATA_LENGTH);
#else
...
void qma6988_calc_press(void)
{
	...
	{
#if defined(USE_SPI)
		qst_spi_read(0xF3, &reg_data[0], 1);
#else
		ret = qst_iic_read((0x70<<1), 0xF3, &reg_data[0], 1);
#endif
		if(reg_data[0]&0x08)
		{
